"""Build this study's figures from the recorded results, at publication quality.

`paper/PLAN.md` allows five figures and no more. Each is built here from a results file
rather than by hand, so a figure cannot drift away from the number it depicts.
...
Figure 1 is the architecture diagram, which is drawn rather than plotted and stays where it
is in `presentation/figures/architecture.png`.
...
Usage:
    python tools/paper_figures.py            # build all
    python tools/paper_figures.py --only 2
"""
